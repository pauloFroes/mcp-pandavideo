...
export function registerFolderTools(server) {
    server.registerTool("list_folders", {
        title: "List Folders",
        description: "List all folders in your PandaVideo account. Optionally filter by parent folder or status.",
        inputSchema: {
            parent_folder_id: z
                .string()
                .uuid()
        ...
        try {
            const data = await apiRequest("/folders", "GET", undefined, { parent_folder_id, status }, BASE_URL_GLOBAL);
    ...
    server.registerTool("get_folder", {
        ...
        description: "Get complete details of a specific folder including its videos, funnels, and DRM groups.",
        inputSchema: {
        ...
        try {
            const data = await apiRequest(`/folders/${folder_id}`, "GET", undefined, undefined, BASE_URL_GLOBAL);
            return toolResult(data);
        }
        catch (error) {
    ...
    server.registerTool("update_folder", {
        ...
        try {
            ...
            if (status !== undefined)
                body.status = status;
            const data = await apiRequest(`/folders/${folder_id}`, "PUT", body, undefined, BASE_URL_GLOBAL);
            return toolResult(data);
        }
        catch (error) {
    ...
    server.registerTool("delete_folder", {
        title: "Delete Folder",
        description: "Soft-delete a folder. Videos inside may be moved to root or deleted depending on configuration.",
        inputSchema: {
