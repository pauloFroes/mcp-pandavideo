...
export function registerPlaylistTools(server: McpServer) {
  server.registerTool(
    ...
    async () => {
      try {
        ...
        return toolError(`Failed to list playlists: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ playlist_id }) => {
      try {
        ...
        return toolError(`Failed to get playlist: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ name, access_type, creator_name, description, password, hide_views }) => {
      try {
        const body: Record<string, unknown> = { name, access_type, creator_name };
        if (description !== undefined) body.description = description;
        if (password !== undefined) body.password = password;
        if (hide_views !== undefined) body.hide_views = hide_views;

        const data = await apiRequest("/playlists", "POST", body, undefined, BASE_URL_GLOBAL);
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to create playlist: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ playlist_id, name, access_type, creator_name, description, password, hide_views }) => {
      try {
        ...
        if (access_type !== undefined) body.access_type = access_type;
        if (creator_name !== undefined) body.creator_name = creator_name;
        if (description !== undefined) body.description = description;
        if (password !== undefined) body.password = password;
        if (hide_views !== undefined) body.hide_views = hide_views;
        ...
        return toolError(`Failed to update playlist: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "delete_playlist",
    {
      ...
      description:
        "Permanently delete a playlist. This action cannot be undone (not a soft delete).",
      inputSchema: {
    ...
    async ({ playlist_id }) => {
      try {
        ...
        return toolError(`Failed to delete playlist: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ playlist_id }) => {
      try {
        ...
        return toolError(`Failed to get playlist videos: ${(error as Error).message}`);
      }
    },
  );
}
