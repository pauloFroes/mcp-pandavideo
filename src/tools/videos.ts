...
export function registerVideoTools(server: McpServer) {
  server.registerTool(
    "list_videos",
    {
      title: "List Videos",
      description:
        "List all videos in your PandaVideo library. Supports pagination, filtering by folder, status, and title search.",
      inputSchema: {
    ...
    async ({ page, limit, title, status, folder_id, root_folder }) => {
      try {
        ...
        return toolError(`Failed to list videos: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "get_video",
    {
      ...
      description:
        "Get complete details of a specific video including title, description, status, storage size, duration, player URL, HLS URL, and thumbnail.",
      inputSchema: {
    ...
    async ({ video_id }) => {
      try {
        ...
        return toolError(`Failed to get video: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "update_video",
    {
      title: "Update Video",
      description:
        "Update video properties such as title, description, folder, or playback qualities.",
      inputSchema: {
    ...
    async ({ video_id, title, description, folder_id, playback }) => {
      try {
        ...
        if (playback !== undefined) body.playback = playback;

        const data = await apiRequest(`/videos/${video_id}`, "PUT", body);
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to update video: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "delete_video",
    {
      title: "Delete Video",
      description:
        "Soft-delete a video. The video will have status DELETING and can be recovered within 30 days using recover_video.",
      inputSchema: {
    ...
    async ({ video_id }) => {
      try {
        ...
        return toolError(`Failed to delete video: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "recover_video",
    {
      ...
      description:
        "Recover a previously deleted video (within 30-day retention window).",
      inputSchema: {
    ...
    async ({ video_id }) => {
      try {
        ...
        return toolResult({ recovered: true, id: video_id, ...((data as Record<string, unknown>) || {}) });
      } catch (error) {
        return toolError(`Failed to recover video: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "upload_video_from_url",
    {
      ...
      description:
        "Import a video from an external URL (must be hosted on a cloud server). Returns a websocket URL to track upload progress.",
      inputSchema: {
    ...
    async ({ url, title, description, folder_id, video_id }) => {
      try {
        ...
        if (description !== undefined) body.description = description;
        if (folder_id !== undefined) body.folder_id = folder_id;
        if (video_id !== undefined) body.video_id = video_id;

        const data = await apiRequest("/videos/", "POST", body, undefined, IMPORT_URL);
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to upload video from URL: ${(error as Error).message}`);
      }
    },
  );

  server.registerTool(
    "upload_video_from_m3u8",
    {
      title: "Upload Video from M3U8",
      description:
        "Import a video from an M3U8/HLS stream URL. Processing is asynchronous.",
      inputSchema: {
    ...
    async ({ url, title, description, folder_id, video_id, hls_encryption }) => {
      try {
        ...
        if (folder_id !== undefined) body.folder_id = folder_id;
        if (video_id !== undefined) body.video_id = video_id;
        if (hls_encryption !== undefined) body.hls_encryption = hls_encryption;

        const data = await apiRequest("/videos/m3u8", "POST", body, undefined, IMPORT_URL);
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to upload video from M3U8: ${(error as Error).message}`);
      }
    },
  );
}
